#!/usr/bin/env bash
# GFS Stack Installer — runs from ~/GhostGoat/
# ./install_gfs.sh

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
GFS_DIR="$SCRIPT_DIR/gfs"
mkdir -p "$GFS_DIR"
echo "[GFS] Installing into $GFS_DIR"

cat > "$GFS_DIR/gfs_registry.py" << 'GFSEOF'
#!/usr/bin/env python3
"""
...
cat > "$GFS_DIR/gfs_logging.py" << 'GFSEOF'
#!/usr/bin/env python3
"""
GFS Logging System
...
cat > "$GFS_DIR/gfs_agent_byte.py" << 'GFSEOF'
#!/usr/bin/env python3
"""
...
cat > "$GFS_DIR/gfs_mcp_bench.py" << 'GFSEOF'
#!/usr/bin/env python3
"""
...
echo ""
echo "[GFS] Stack ready at $GFS_DIR"
ls -lh "$GFS_DIR"/*.py