...
from config.unified_config import MemoryBackend, MemoryConfig, get_config
...
class InMemoryBackend(MemoryInterface):
    ...
    async def retrieve(self, query: str, top_k: int = 5) -> List[MemoryItem]:
        """Retrieve memories (simple substring match)"""
        query_lower = query.lower()
        matches = []
...
class UnifiedMemory:
    ...
    def _initialize_backend(self):
        ...
        elif self.config.backend == MemoryBackend.MEMORY:
            self.backend = InMemoryBackend(self.config)
        else:
            logger.warning(f"Unknown backend {self.config.backend}, using in-memory")
            self.config.backend = MemoryBackend.MEMORY
            self.backend = InMemoryBackend(self.config)
