"""GhostGoat GraphRAG Engine — Robust Semantic Vector Graph."""
import json, time, uuid, hashlib, logging, os
...
class GraphRAGEngine:
    ...
    def bulk_mode(self, enabled: bool = True):
        if isinstance(enabled, bool):
            ...
            return self
        class _CM:
            def __enter__(cm): self._bulk = True
            def __exit__(cm, *a): self._bulk = False; self._persist()
        return _CM()
    ...
    def search(self, query_vector: List[float], node_type: Optional[str] = None,
        ...
        return [{"id": n.id, "label": n.label, "type": n.node_type,
                 "content": n.content[:300], "similarity": sim,
                 "metadata": n.metadata} for sim, n in scored[:top_k]]

    def traverse(self, start_node_id: str, depth: int = 2,
    ...
    def get_subgraph(self, center_id: str, radius: int = 1) -> Dict[str, Any]:
        ...
        return {
            "nodes": [self.nodes[nid].__dict__ for nid in node_ids if nid in self.nodes],
        ...
        for node in self.nodes.values():
            ...
            if sim > threshold:
                self.edges.append(GraphEdge(
                    source=new_node.id, target=node.id,
                    edge_type="SIMILAR_TO", weight=sim))
                self.adjacency.setdefault(new_node.id, []).append(
                    (node.id, sim, "SIMILAR_TO"))
                self.adjacency.setdefault(node.id, []).append(
                    (new_node.id, sim, "SIMILAR_TO"))
    ...
    def _persist(self):
        try:
            ...
            _atomic_json_write(self.storage_path / "graph.json", data)
            self._dirty = False
    ...
    def _load(self):
        ...
        try:
            ...
            for obj in data.get("edges", []):
                e = GraphEdge(**obj)
                self.edges.append(e)
                self.adjacency.setdefault(e.source, []).append(
                    (e.target, e.weight, e.edge_type))
                self.adjacency.setdefault(e.target, []).append(
                    (e.source, e.weight, e.edge_type))
            logger.info("Graph loaded: %d nodes, %d edges", len(self.nodes), len(self.edges))
        except Exception as e:
            logger.warning("Graph load failed: %s — backing up and starting fresh", e)
