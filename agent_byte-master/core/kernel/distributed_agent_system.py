...
context = system.backend.get_folder_context('{self.folder_path}')
```
...
class DistributedAgentSystem:
    ...
    def scan_and_generate_agents(self, exclude_dirs=None, extensions=None):
        ...
        print(f" Generating AGENT.md in every folder with tracked files...\n")

        # Track folders that need agents
        folders_with_files = set()

        # First pass: index all files into the neural backend
    ...
    def list_agents(self):
        ...
        for agent_id, info in self.backend.agent_registry.items():
            ...
            print()
        return list(self.backend.agent_registry.keys())


if __name__ == "__main__":
    print("=" * 60)
    print("GHOSTGOAT DISTRIBUTED AGENT SYSTEM")
    ...
    print("\n Usage:")
    print("  make agents               — regenerate all AGENT.md files")
    print("  python -m core.distributed_agent_system  — run directly")
