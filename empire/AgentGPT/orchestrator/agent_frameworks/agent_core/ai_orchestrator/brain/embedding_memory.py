import json
import os
# Requires: pip install sentence-transformers
try:
    from sentence_transformers import SentenceTransformer, util 
except ImportError:
    print("Warning: sentence-transformers not installed. Vector Memory will be non-functional.")
    class SentenceTransformer:
        def __init__(self, model): pass
        def encode(self, text): return []
    class util:
        @staticmethod
        def cos_sim(a, b): return [0.0]

class EmbeddingMemory:
    def __init__(self, path="data/memory_embeddings.json"):
        self.path = path
        self.model = SentenceTransformer("all-MiniLM-L6-v2")
        self.memory = self._load()

    def _load(self):
        if os.path.exists(self.path):
            with open(self.path, "r") as f:
                data = json.load(f)
                for item in data:
                    item["embedding"] = item.get("embedding", [])
                return data
        return []

    def save(self):
        with open(self.path, "w") as f:
            json.dump(self.memory, f, indent=2)

    def store(self, user_id, key, text):
        if not text.strip(): return
        
        # Only encode if the model is actually loaded (handles ImportError gracefully)
        if self.model:
            embedding = self.model.encode(text).tolist()
            self.memory.append({"user": user_id, "key": key, "text": text, "embedding": embedding})
            self.save()
        
    def recall(self, query, top_k=3):
        if not self.memory or not self.model: return []
        
        query_vec = self.model.encode(query)
        sims = [
            (util.cos_sim(query_vec, item["embedding"]).item(), item)
            for item in self.memory if item["embedding"]
        ]
        sims.sort(reverse=True, key=lambda x: x[0])
        return [item for _, item in sims[:top_k]]
