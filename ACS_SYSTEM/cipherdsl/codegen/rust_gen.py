...
const PIPELINE_JSON: &str = r#"{PIPELINE_JSON}"#;
"""

def generate_rust_project(steps, outdir):
    # Bake pipeline JSON into Rust source
    pipeline_json = json.dumps(steps, separators=(",",":"))
    main_rs = MAIN_RS_TEMPLATE.replace("{PIPELINE_JSON}", pipeline_json)

    Path(outdir, "src").mkdir(parents=True, exist_ok=True)
    (Path(outdir) / "Cargo.toml").write_text(CARGO_TOML)
    (Path(outdir) / "src" / "main.rs").write_text(main_rs)
