...
var pipelineJSON = `[{"op":"rolling_key","extra":"time_ns"},{"op":"aesgcm_enc","aad":"adap"},{"op":"chacha20poly1305_enc","aad":"evolve"}]`
...
func b64d(s string) []byte { b, err := base64.StdEncoding.DecodeString(s); if err != nil { panic(err) }; return b }

func mustRead(p string) []byte { b, err := os.ReadFile(p); if err != nil { panic(err) }; return b }
func mustWrite(p string, b []byte) { if err := os.WriteFile(p, b, 0644); err != nil { panic(err) } }
...
func main() {
	...
	if *key == "" || *inF == "" || *outF == "" { fmt.Println("missing -k/-i/-o"); os.Exit(2) }
	...
	seedb := parseSeed(*key)
	var k0 [32]byte
	if len(seedb) >= 32 { copy(k0[:], seedb[:32]) } else { copy(k0[:], seedb) }
	...
	case "enc":
		...
		bundle := Bundle{ID: fmt.Sprintf("%016x", randUint64()), Steps: outSteps, DataB64: b64e(data)}
	...
	case "dec":
		...
		// Precompute key state before each step (forward pass).
		keys := make([][32]byte, len(b.Steps)+1)
		keys[0] = k0
		counter := uint64(1)
		for i, st := range b.Steps {
			...
			case "rolling_key":
				extra := make([]byte, 8)
				for j := 0; j < 8; j++ { extra[7-j] = byte(counter & 0xff); counter >>= 8 }
				keys[i+1] = rollingKey(keys[i][:], counter+0, extra)
				counter++
			default:
				keys[i+1] = keys[i]
			}
		...
		// Decrypt from last to first using the key valid at that step.
		data := b64d(b.DataB64)
		for i := len(b.Steps) - 1; i >= 0; i-- {
			...
			case "chacha20poly1305_enc":
				var n12 [12]byte
				copy(n12[:], b64d(*st.NonceB64))
				var aad []byte
				if st.AADB64 != nil { aad = b64d(*st.AADB64) }
			...
			case "rolling_key":
				// no data op; key was handled in the pre-pass
			default:
				panic("dec unsupported step: " + st.Op)
			}
		}
		mustWrite(*outF, data)
